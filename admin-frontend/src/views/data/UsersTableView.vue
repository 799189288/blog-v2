...
const columns: DataTableColumns<UserRow> = [
  ...
  { title: 'Username', key: 'username', sorter: true, render: r => h('a', { onClick: () => openDetail(r), style: 'cursor:pointer' }, r.username) },
  { title: 'Role', key: 'role', width: 120, sorter: true, render: r => h(NTag, { size: 'small', type: 'info' }, () => r.role) },
  { title: 'Created', key: 'created_at', width: 160, sorter: true, render: r => dayjs(r.created_at).format('YYYY-MM-DD HH:mm') },
]
</script>

<template>
  <h2 style="margin-top: 0">Users (data view)</h2>
  ...
  />
  <RowDetailDrawer v-model:show="drawer" title="User row" :data="detail" />
</template>
